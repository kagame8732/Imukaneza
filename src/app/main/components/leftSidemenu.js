...
const LeftSideMenu = (props) => {
    ...
    return (
        ...
        img src = { Logo }
        alt = ""
        id = "left_side_admin_logo" / >
        <
        p id = "name_web_left_side" > imukaneza < /p> { /* navigation style  */ } {
            navigations.map((item, key) => {
                return <Navigation name = { item.name }
                url = { item.url }
                theOne = { props.whichOne }
                icon = { item.icon }
                key = { key }
                />
            })
        } {
            props.isPhone === true ? < img src = { img_doc }
            id = "admin_img_style_phone"
            alt = "" / >:
                <
                img src = { img_doc }
            id = "admin_img_style"
